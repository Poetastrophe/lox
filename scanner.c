...
static void scanner_error(Scanner *scanner, char *message) {
  ...
  scanner->errors[scanner->error_count++] = serror;
}
...
static Token *scanner_string(Scanner *scanner) {
  ...
  size_t str_len = scanner->buffer - start;
  char *str = malloc((str_len + 1) * sizeof(char));
  strncpy(str, start, str_len);
...
static Token *scanner_number(Scanner *scanner) {
  int column = scanner->column - 1;
  char *number_start = scanner->buffer - 1;
  ...
  size_t num_str_len = scanner->buffer - number_start;
  char *num_str = malloc((num_str_len + 1) * sizeof(char));
  strncpy(num_str, number_start, num_str_len);

  double *parsed_number = malloc(sizeof(double));
  *parsed_number = atof(num_str);
  Literal *number_literal = literal_new(TYPE_NUMBER, parsed_number);
  return token_new(NUMBER, scanner->line, column, num_str, number_literal);
}

static ScannerResult *scannerresult_init() {
  ScannerResult *result = malloc(sizeof(ScannerResult));
  result->scanner_error_count = 0;
  result->token_capacity = 8;
  result->tokens = malloc(result->token_capacity * sizeof(Token *));
  result->scanner_errors = malloc(8 * sizeof(ScannerError *));
  return result;
}

static void scanner_free(Scanner *scanner) { free(scanner); }

static void scanner_print_error(ScannerError *error) {
...
static Token *scanner_new_token(TokenType type, Scanner *scanner,
  ...
  return token_new(type, scanner->line, scanner->column - lexeme_size, lexeme,
                   NULL);
}
...
static Token *scanner_identifier(Scanner *scanner) {
  ...
  Literal *ident_literal = literal_new(TYPE_STRING, identifier);
  return token_new(IDENTIFIER, scanner->line, column, identifier,
                   ident_literal);
}
...
static Token *scanner_scan_token(Scanner *scanner) {
  ...
  while (!scanner_is_at_end(scanner)) {
    ...
    case '-':
      return scanner_new_token(MINUS, scanner, "-");
    case '+':
      return scanner_new_token(PLUS, scanner, "+");
    case ';':
...
ScannerResult *scanner_run(Scanner *scanner) {
  ...
  while (!scanner_is_at_end(scanner)) {
    ...
    if (index > result->token_capacity) {
      result->token_capacity = result->token_capacity * 2;
      Token **new_token_block =
          malloc(sizeof(Token *) * result->token_capacity);
      for (size_t i = 0; i < index; i++) {
        new_token_block[i] = result->tokens[i];
      }
      free(result->tokens);
      result->tokens = new_token_block;
    }
    result->tokens[index++] = token;
  }
  result->scanner_error_count = scanner->error_count;
  result->scanner_errors = scanner->errors;
...
Scanner *scanner_from_file(const char *filename) {
  ...
  scanner->buffer = malloc(sizeof(char) * (file_size + 1));
  rewind(file);
...
void scanner_print_errors(ScannerResult *result) {
  for (size_t i = 0; i < result->scanner_error_count; i++)
    scanner_print_error(result->scanner_errors[i]);
}
